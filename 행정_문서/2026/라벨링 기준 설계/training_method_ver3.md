...
|---|---:|---:|---:|---:|
| Train | 2096 | 1695.98px | 1704.53px | 1877.76px |
...
## YOLO11 Fine-Tuning Method Analysis

### Fine-Tuning Strategy

현재 학습은 Ultralytics YOLO GitHub 기본 학습 방식을 그대로 사용하고 있습니다.
...
위 방식에서 `yolo11s-seg.pt`는 COCO 등 대규모 데이터셋으로 사전학습(pretrained)된 모델입니다.

학습 시작 시:

1. 사전학습 weight를 초기값으로 로드한다.
2. 현재 데이터셋 클래스 수와 맞지 않는 출력 layer는 자동으로 재초기화된다.
3. 별도의 freeze 옵션이 없으면 backbone, neck, head 전체가 학습된다.

즉 현재 실험은 다음과 같이 분류할 수 있다.

| Item | Status |
|---|---|
| Pretrained Weight 사용 | O |
| Transfer Learning | O |
| Full Fine-Tuning | O |
| Partial Fine-Tuning | X |
| Layer Freeze | X |

따라서 현재 방식은 "일부 layer만 재학습"이 아니라 **전체 모델 Full Fine-Tuning** 방식이다.

### Loss Function Structure

YOLO11 Segmentation 학습은 detection loss와 segmentation loss를 함께 최적화한다.
...
| Loss | Purpose |
|---|---|
| box_loss | Bounding Box 위치 오차 |
| cls_loss | 객체 클래스 분류 오차 |
| dfl_loss | Distribution Focal Loss 기반 bbox 회귀 |
| seg_loss | Instance Segmentation Mask 오차 |
| sem_loss | Semantic Segmentation Head Loss (사용 시) |

### Total Training Objective

개념적으로 전체 loss는 다음과 같이 표현할 수 있다.
...
실제 내부 구현에서는 각 항목에 가중치가 적용된다.

### Segmentation Loss

Instance Segmentation은 Prototype Mask 기반 구조를 사용한다.
...
Mask loss는 Binary Cross Entropy(BCE) 기반으로 계산된다.
...
- COCO pretrained segmentation 모델 사용
- 철스크랩 데이터셋으로 transfer learning 수행
- Backbone 포함 전체 weight 업데이트
- Detection과 Segmentation을 동시에 최적화
- Small-object filtering 효과를 loss 기반으로 학습

따라서 현재 실험은 "처음부터 학습(training from scratch)"이 아니라,

**YOLO11 사전학습 모델을 기반으로 한 Full Fine-Tuning 기반 Instance Segmentation 학습**으로 정의할 수 있다.

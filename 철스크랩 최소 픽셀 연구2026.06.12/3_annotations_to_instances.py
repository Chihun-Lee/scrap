...
# instances_val.json 기준의 고정 category id 순서
...
def normalize_label(label):
    s = re.sub(r"^\s*\d+\s*[\.\-:_]?\s*", "", str(label))
    return s.strip()


def get_canonical_categories():
    return [
        {"id": idx, "name": name, "supercategory": ""}
        for idx, name in enumerate(CANONICAL_CATEGORIES, start=1)
    ]


CANONICAL_NAME_TO_ID = {
    normalize_label(name).lower(): idx
    for idx, name in enumerate(CANONICAL_CATEGORIES, start=1)
}
...
def find_image_for_json(json_file, data):
    ...
    if image_path:
        candidate = os.path.normpath(os.path.join(json_dir, image_path))
    ...
    candidates = glob.glob(os.path.join(json_dir, base + ".*"))
...
def calculate_polygon_area_anylabeling(segmentations):
    ...
    for points in valid_segmentations:
        ...
        # AnyLabeling은 round 후 polygon rasterize
        ...
        if cv2 is not None:
            ...
            draw.polygon([tuple(pt) for pt in points_int.tolist()], fill=1, outline=1)
...
def convert_labelme_dir_to_coco_anylabeling(input_dir, output_json):
    ...
    coco["categories"] = get_canonical_categories()
    ...
    for json_file in json_files:
        ...
        image_path = find_image_for_json(json_file, data)
        ...
        for shape in data.get("shapes", []):
            ...
            if label_key not in CANONICAL_NAME_TO_ID:
                raise ValueError(
                    f"Unknown category label '{label}' in {json_file}. "
                    "Add it to CANONICAL_CATEGORIES before conversion."
                )

            category_id = CANONICAL_NAME_TO_ID[label_key]

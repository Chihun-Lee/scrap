#!/bin/bash
# 클러스터 학습 결과 자동 회수 (맥북 launchd, 30분 주기) — 2026-07-15, 경로 갱신 2026-09-22
# 오프라인이면 조용히 종료, 연결되면 결과만 증분 rsync. 중지: launchctl unload ~/Library/LaunchAgents/com.chihun.scrap-pull-results.plist
LOCAL="/Users/chihun/Code/철스크랩/2_코드/라벨링기준연구/실험스크립트"
DEST="/Users/chihun/Code/철스크랩/3_결과/클러스터_회수"
...
if [ "$done_n" -ge 3 ] && [ ! -f "$DEST/.notified" ]; then
  osascript -e 'display notification "exp2·exp6·exp7 학습 결과 회수 완료 (3_결과/클러스터_회수/)" with title "철스크랩 학습 완료"' 2>/dev/null
  touch "$DEST/.notified"
fi
